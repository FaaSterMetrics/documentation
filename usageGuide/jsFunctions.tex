...
Within the wrapped user function, this can easily be achieved via the \mintinline{js}{ctx} function parameter.
...
Also, since functions are typically asynchronous, it may be necessary to \mintinline{js}{await} those calls.
...
% TODO Question: can we call router functions?
...
Values can be strings, arrays or JavaScript objects. %TODO ask Christoph: Should all be stringified, right?
...
%TODO ask if ctx.log should be awaited
\begin{figure}
\begin{tcolorbox}
\begin{minted}{js}
  const lib = require('@faastermetrics/lib')
