...
\section{Instrumentation of Functions}%
\label{sec:functionInstrumentation}
...
To estimated the impact and overhead of logging, we benchmarked AWS, Azure and GCP printing performance. 
...
TODO temporal shift
%\subsubsection{Function Temporal Shift}%
%\label{ssub:functionTemporalShift}
%
%When a function under a context gets called multiple times, 
%the execution can happen on multiple executors with different system clocks, making the temporal order impossible to detect.
%Thus adding an executor context to an event will help to distinguish the temporal order on a single executor but not in the global context. 
%A vector clock can help to make each execution thread distinguishable and temporally orderable. 
%Implementing a vector clock is not trivial and depends on the program size, thus not constant in size. 
%Adding a variable size header to each request would modify the experiment results depending on the program. 
%Thus we have decided not to implement a vector clock and evaluate the design decision after the first experiments.

\subsubsection{Runtime probes}%
\label{ssub:runtimeProbes}
...
To measure the actual runtime of a function we set time probes at the beginning and at the end of a function which will generate an event. 
This probe has to be present in every function, so we insert our measurement into functions as shown
in \Cref{fig:timeMeasurementCode}. TODO

\subsubsection{Calling Time Probes}%
\label{ssub:callingTimeProbes}
...
TODO: ``?''
In the global configuration, a logging command is specified, which will output all logs for the given experiment. 
This ia implemented by specifying identifier tags/prefixes for logging, which are part of the platform-specific configuration.
